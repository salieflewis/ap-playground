...
var getUserDefinedScalarTypes = (schema) => {
  return Object.values(schema.getTypeMap()).filter(
    (type) => !!type.astNode && type.astNode.kind === Kind.SCALAR_TYPE_DEFINITION && !["BigInt", "Bytes"].includes(type.name)
  );
};
...
import { GraphQLObjectType as GraphQLObjectType5, GraphQLSchema as GraphQLSchema2 } from "graphql";
...
var buildPluralField = ({
  ...
  entity.fields.forEach((field) => {
    switch (field.kind) {
      ...
      case "RELATIONSHIP": {
        ...
        operators.singular.forEach((suffix) => {
          filterFields[`${field.name}${suffix}`] = {
            type: new GraphQLList2(field.relatedEntityIdType)
          };
        });
  ...
  return {
    type: new GraphQLNonNull2(
      new GraphQLList2(new GraphQLNonNull2(entityGqlType))
    ),
    args: {
...
var buildSingularField = ({
  ...
  return {
    type: entityGqlType,
    args: {
      id: { type: new GraphQLNonNull3(entity.fieldByName.id.scalarGqlType) }
    },
    resolve: resolver
  };
};
...
var buildGqlSchema = (schema) => {
  ...
  const gqlSchema = new GraphQLSchema2({
    query: queryType
  });
  return gqlSchema;
};
    ...
    return `export type ${entity.name} = {
      ...
      switch (field.kind) {
        ...
        case "LIST": {
          ...
          } else if (field.baseGqlType.astNode?.kind === Kind2.ENUM_TYPE_DEFINITION) {
...
// src/codegen/service.ts
var CodegenService = class extends Emittery2 {
  resources;
  contracts;
  logFilters;
  constructor({
  ...
  generateAppFile({ schema } = {}) {
    ...
    const filePath = path4.join(this.resources.options.generatedDir, "index.ts");
...
var buildSingleAbi = ({
  ...
  if (typeof abiConfig === "string") {
    filePath = path5.isAbsolute(abiConfig) ? abiConfig : path5.join(path5.dirname(configFilePath), abiConfig);
    const abiString = readFileSync3(filePath, "utf-8");
...
var buildDatabase = ({
  ...
  const defaultSqliteFilename = path6.join(options.ponderDir, "cache.db");
  if (config.database) {
...
var UserErrorService = class extends Emittery3 {
  hasUserError = false;
...
var EventAggregatorService = class extends Emittery4 {
  eventStore;
  logFilters;
  networks;
...
// src/historical-sync/service.ts
var HistoricalSyncService = class extends Emittery5 {
  resources;
  eventStore;
  logFilters;
  network;
  ...
  buildQueue = () => {
    ...
    const queue = createQueue({
      ...
      onError: ({ error, task, queue: queue2 }) => {
        ...
        if (task.kind === "LOG_SYNC" && error instanceof InvalidParamsRpcError && error.details.includes("block range less than 20000")) {
...
var LoggerService = class {
  ...
  constructor({
    ...
    if (dir) {
      const logFile = path7.join(dir, `${(/* @__PURE__ */ new Date()).toISOString()}.log`);
      streams.push({
        level: "trace",
...
// src/realtime-sync/service.ts
var RealtimeSyncService = class extends Emittery6 {
  resources;
  eventStore;
  logFilters;
  network;
  ...
  setup = async () => {
    ...
    this.resources.logger.info({
      service: "realtime",
      msg: `Fetched latest block at ${hexToNumber4(
        latestBlock.number
      )} (network=${this.network.name})`
    });
    ...
    const priority = Number.MAX_SAFE_INTEGER - hexToNumber4(latestBlock.number);
  ...
  start = async () => {
    ...
    this.resources.logger.info({
      service: "realtime",
      msg: `Fetched finalized block at ${hexToNumber4(
  ...
  addNewLatestBlock = async () => {
    ...
    const priority = Number.MAX_SAFE_INTEGER - hexToNumber4(block.number);
  ...
  blockTaskWorker = async (block) => {
    ...
    if (newBlock.number > previousHeadBlock.number + 1) {
      ...
      for (const block2 of [...missingBlocks, newBlockWithTransactions]) {
        const priority = Number.MAX_SAFE_INTEGER - hexToNumber4(block2.number);
...
var ServerService = class {
  ...
  async start() {
    ...
    const server = await new Promise((resolve, reject) => {
      const server2 = createServer(this.app).on("error", (error) => {
...
var HandlersBar = ({ ui }) => {
  ...
  const titleText = () => {
    if (isUpToDate)
      return /* @__PURE__ */ React2.createElement(Text2, { color: "green" }, "(up to date)");
    if (isStarted)
      return /* @__PURE__ */ React2.createElement(Text2, { color: "yellow" }, "(up to ", ui.handlersToTimestamp === 0 ? "" : dateText, ")");
  ...
  return /* @__PURE__ */ React2.createElement(Box, { flexDirection: "column" }, /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row" }, /* @__PURE__ */ React2.createElement(Text2, { bold: true }, "Event handlers "), /* @__PURE__ */ React2.createElement(Text2, null, titleText())), /* @__PURE__ */ React2.createElement(Box, { flexDirection: "row" }, /* @__PURE__ */ React2.createElement(
    ProgressBar,
    {
...
var HistoricalBar = ({
  ...
  return /* @__PURE__ */ React3.createElement(Box2, { flexDirection: "column" }, /* @__PURE__ */ React3.createElement(Text3, null, title), /* @__PURE__ */ React3.createElement(Box2, { flexDirection: "row" }, /* @__PURE__ */ React3.createElement(ProgressBar, { current: rate, end: 1 }), /* @__PURE__ */ React3.createElement(Text3, null, " ", rateText, etaText)));
};
...
var getStackTrace = (error, options) => {
  if (!error.stack)
    return void 0;
  const buildDir = path8.join(options.ponderDir, "out");
...
function getSourceMappedStackFrame(file, lineNumber, columnNumber) {
  ...
  if (!sourceFileRelative || !sourceLineNumber)
    return null;
  const sourceFile = path8.resolve(path8.dirname(file), sourceFileRelative);
  return {
    sourceFile,
    sourceLineNumber,
    sourceColumnNumber,
...
// src/user-handlers/service.ts
var EventHandlerService = class extends Emittery7 {
  resources;
  userStore;
...
//# sourceMappingURL=chunk-E2PLELPX.mjs.map