...
var buildOptions = ({
  ...
  const defaults = {
    rootDir: import_node_path2.default.resolve(cliOptions.rootDir),
  ...
  return {
    ...defaults,
    // Resolve paths
    configFile: import_node_path2.default.join(defaults.rootDir, defaults.configFile),
    schemaFile: import_node_path2.default.join(defaults.rootDir, defaults.schemaFile),
    srcDir: import_node_path2.default.join(defaults.rootDir, defaults.srcDir),
    generatedDir: import_node_path2.default.join(defaults.rootDir, defaults.generatedDir),
    ponderDir: import_node_path2.default.join(defaults.rootDir, defaults.ponderDir),
    logDir: import_node_path2.default.join(defaults.rootDir, defaults.logDir)
  };
};
...
var readHandlers = async ({ options }) => {
  const entryAppFilename = import_node_path3.default.join(options.generatedDir, "index.ts");
  if (!(0, import_node_fs3.existsSync)(entryAppFilename)) {
    throw new Error(
  ...
  const buildDir = import_node_path3.default.join(options.ponderDir, "out");
  (0, import_node_fs3.rmSync)(buildDir, { recursive: true, force: true });
  try {
  ...
  const tsconfigPath = import_node_path3.default.join(options.rootDir, "tsconfig.json");
  if ((0, import_node_fs3.existsSync)(tsconfigPath)) {
  ...
  const outAppFilename = import_node_path3.default.join(buildDir, "generated/index.js");
...
var CodegenService = class extends import_emittery2.default {
  ...
  generateAppFile({ schema } = {}) {
    ...
    const filePath = import_node_path5.default.join(this.resources.options.generatedDir, "index.ts");
    ensureDirExists(filePath);
    (0, import_node_fs6.writeFileSync)(filePath, final, "utf8");
    this.resources.logger.debug({
      service: "codegen",
...
var buildSingleAbi = ({
  ...
  if (typeof abiConfig === "string") {
    filePath = import_node_path6.default.isAbsolute(abiConfig) ? abiConfig : import_node_path6.default.join(import_node_path6.default.dirname(configFilePath), abiConfig);
    const abiString = (0, import_node_fs7.readFileSync)(filePath, "utf-8");
...
var buildDatabase = ({
  ...
  const defaultSqliteFilename = import_node_path7.default.join(options.ponderDir, "cache.db");
  if (config.database) {
...
var LoggerService = class {
  ...
  constructor({
    ...
    if (dir) {
      const logFile = import_node_path8.default.join(dir, `${(/* @__PURE__ */ new Date()).toISOString()}.log`);
      streams.push({
        level: "trace",
  ...
  async start() {
    ...
    this.app.use((req, res, next) => {
      ...
      res.on("finish", () => {
        ...
        const path13 = new URL(req.url, `http://${req.get("host")}`).pathname;
...
var getStackTrace = (error, options) => {
  if (!error.stack)
    return void 0;
  const buildDir = import_node_path9.default.join(options.ponderDir, "out");
...
function getSourceMappedStackFrame(file, lineNumber, columnNumber) {
  ...
  if (!sourceFileRelative || !sourceLineNumber)
    return null;
  const sourceFile = import_node_path9.default.resolve(import_node_path9.default.dirname(file), sourceFileRelative);
  return {
    sourceFile,
    sourceLineNumber,
    sourceColumnNumber,
...
var Ponder = class {
  ...
  async setup() {
    this.resources.logger.debug({
      service: "app",
      msg: `Started using config file: ${import_node_path10.default.relative(
...
cli.command("dev", "Start the development server").action(async (cliOptions) => {
  if (cliOptions.help)
    process.exit(0);
  const configFile = import_node_path11.default.resolve(cliOptions.configFile);
